...
Write-Host "CORE AWAKENED - Integracion XENOID V3" -ForegroundColor Cyan
Write-Host "Proyecto: $ProjectRoot"

$Backup = Join-Path $ProjectRoot ("backup_pre_xenoid_" + (Get-Date -Format 'yyyyMMdd_HHmmss'))
New-Item -ItemType Directory -Force -Path $Backup | Out-Null
foreach ($rel in @('scripts/main.gd','scripts/fighter.gd','scripts/presentacion_vs.gd','scripts/resultado.gd','scripts/game_state.gd','scripts/selector_personajes.gd','scripts/xenoid.gd')) {
...
$fighter = Join-Path $ProjectRoot 'scripts/fighter.gd'

# FASE XENOID V3 — cada personaje puede mover verticalmente la gigantografía
# del Absoluto sin alterar el tamaño corporal ni el comportamiento del resto.
$old = "var textura_absoluto: Texture2D = null" + $NL + "var textura_recarga: Texture2D = null"
$new = "var textura_absoluto: Texture2D = null" + $NL + "# Offset vertical opcional del póster del tercer CORE. Negativo = más arriba." + $NL + "var offset_poster_absoluto_y: float = 0.0" + $NL + "var textura_recarga: Texture2D = null"
Replace-Literal $fighter $old $new 'Soporte offset gigantografia Absoluto'

$old = "`tvar pos_base := Vector2(-mirando * 18.0, -(float(tex.get_height()) * esc) / 2.0 - 18.0)" + $NL + "`timg.position = pos_base" + $NL + "`t# El póster se ve como arte integrado detrás del personaje." + $NL + "`t# No usamos ninguna placa/rectángulo: el oscurecimiento vive en el propio" + $NL + "`t# póster. Ahora tiene más presencia y menos transparencia, y el Absoluto" + $NL + "`t# recibe un poco más de fuerza visual que los poderes 1/2." + $NL + "`tvar es_absoluto: bool = tex == textura_absoluto"
$new = "`tvar pos_base := Vector2(-mirando * 18.0, -(float(tex.get_height()) * esc) / 2.0 - 18.0)" + $NL + "`tvar es_absoluto: bool = tex == textura_absoluto" + $NL + "`tif es_absoluto:" + $NL + "`t`tpos_base.y += offset_poster_absoluto_y" + $NL + "`timg.position = pos_base" + $NL + "`t# El póster se ve como arte integrado detrás del personaje." + $NL + "`t# No usamos ninguna placa/rectángulo: el oscurecimiento vive en el propio" + $NL + "`t# póster. Ahora tiene más presencia y menos transparencia, y el Absoluto" + $NL + "`t# recibe un poco más de fuerza visual que los poderes 1/2."
Replace-Literal $fighter $old $new 'Posicion vertical gigantografia Absoluto'
...
Write-Host "Abri Godot y deja que importe los PNG nuevos. Proba Xenoid: caminata, bloqueo, carrera, recarga, combo de 6 golpes y Absoluto con gigantografia arriba. Varkhos sigue bloqueado." -ForegroundColor Cyan
